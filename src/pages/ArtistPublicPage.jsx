...
import {
  ArrowLeft, CalendarDays, ExternalLink, Globe, Instagram,
...
export default function ArtistPublicPage() {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .limit(60)
      .then(({ data }) => {
        setEvents((data || []).filter((event) => lineupIncludesArtist(event.lineup, artist)).slice(0, 6));
      });
  }, [artist]);
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[11px] text-white/35 truncate">
                          {[event.venue || event.city, new Date(event.date).toLocaleDateString('es-CO', { day: 'numeric', month: 'short' })].filter(Boolean).join(' · ')}
                        </p>
                      </div>
