...
test('las pestañas varían por tipo de perfil, no las mismas 4 para todos', () => {
  ...
  assert.match(profileContentTabs, /collective:\s*\{ events: true, music: false, podcast: true,\s*interviews: true \}/);
});

test('un organizador con fila espejo en artists nunca muestra Música, aunque tenga artistId', () => {
  // El gate requiresArtist por sí solo no bastaba: un club/colectivo con
  // mirror artist (sube música/podcast propios) pasaba artistId, y antes
  // eso alcanzaba para mostrar Música también — capabilities lo bloquea
  // explícitamente sin importar si hay artistId.
