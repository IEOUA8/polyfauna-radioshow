-- POLYFAUNA — autolimpieza de checkouts abandonados
-- Un checkout "pending" sin wompi_transaction_id nunca llegó a completarse en
-- el widget de Wompi (el comprador lo cerró antes de pagar). No hay riesgo de
-- perder un pago porque Wompi nunca le asignó un ID de transacción. Se marcan
-- como 'voided' automáticamente para que la alerta de "pagos pendientes
-- antiguos" quede reservada a checkouts que sí iniciaron pago en Wompi y
-- podrían tener un webhook perdido.
...
BEGIN
  ...
  SELECT
    ...
    'Hay checkouts pendientes con más de 45 minutos que sí iniciaron pago en Wompi.'::TEXT,
