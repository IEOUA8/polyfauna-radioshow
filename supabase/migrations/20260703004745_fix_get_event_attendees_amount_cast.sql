-- get_event_attendees tambien fallaba (42804, "structure of query does not
-- match function result type") por transactions.amount_total: es BIGINT en
-- la tabla pero la funcion declara NUMERIC. Esto es preexistente, no lo
-- introdujo el cambio de assigned_email; probablemente nunca se probo con
-- datos reales antes de hoy. Cast explicito a numeric.
...
BEGIN
  ...
  SELECT ticket.id, ticket.ticket_number, ticket.ticket_type, ticket.status, ticket.created_at, ticket.user_id,
    profile.display_name, profile.phone, COALESCE(users.email::TEXT, ticket.assigned_email), transaction.wompi_reference, transaction.amount_total::NUMERIC,
